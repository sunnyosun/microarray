## Feature Extraction of micriarray data ##
# by Sunny #
...
# Before running the function, make sure to put the "SK1rosetta.txt" file into your working 
# directory, the "SK1rosetta.txt" file can be found in WD_Drive/R_files/FE folder.
...
FE<-function(FileName, cy0, strain, range=NULL)
 {
   ...
   chr<-function(x) which (plat[,'chr']==x)
   chrorder<-function(x)chr(x)[order(plat[which(plat[,'chr']==x),'start'])]
   ...
   if (is.null(range)==FALSE)
   {
     for (i in 1:16)
     {
       ...
       plot(plat[chrorder(i),'start']/1000,2^(lr[chrorder(i)]),
...
FE_PAIR<-function(FileName_1, cy0_1, strain_1, FileName_2, cy0_2, strain_2, range=NULL)
{
  ...
  if (is.null(range)==FALSE)
  {
    for (i in 1:16)
    {
      ...
      par(mfrow=c(2,1))
      plot(plat[chrorder(i),'start']/1000,2^(lr1[chrorder(i)]),
           xlab=paste('Chromosome',i,' Position (kb)', sep=""), 
           ylab='Signal', frame.plot=F,pch=16, col='blue', main=strain_1,ylim=c(0,range))
      plot(plat[chrorder(i),'start']/1000,2^(lr2[chrorder(i)]),
           xlab=paste('Chromosome',i,' Position (kb)', sep=""), 
